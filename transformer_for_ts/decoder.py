...
class Decoder(nn.Module):
    ...
    def __init__(self,
        ...
        self._dopout3 = nn.Dropout(p=dropout)

    def forward(self, x: torch.Tensor, memory: torch.Tensor, word_length: torch.Tensor) -> torch.Tensor:
        ...
        residual = x
        x = self._encoderDecoderAttention(query=x, key=memory, value=memory, word_length=word_length, decoder=0)
        x = self._dopout2(x)
